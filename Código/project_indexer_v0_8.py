...
def index_term(term, newsid, position, index_term2news, index_permuterm):
    ...
    if not newslist:
        ...
        index_news2pos = {newsid:[position]}
        index_term2news[term] = [[newsid], index_news2pos]
    # Si ya hay noticias registradas para el término
    else:
        if newsid in newslist[0]:
            poslist = newslist[1][newsid]
            poslist.append(position)
            newslist[1][newsid] = poslist
        else:
            newslist[0].append(newsid)
            newslist[1][newsid] = [position]
